...
const submitHandlerPlace = ({ name, link }) => {
    ...
    api.addCard({ name, link })
        .then((resp) => {
        cardSection.addItem(createCard(resp))
            popupCardForm.close()
        })
        .catch((err) => {
            console.log(`Ошибка: ${err}`);
...
const popupConfirm = new PopupWithConfirm('.popup-confirm', submitHandlerConfirm);
